...
def facecrop(model, org_path, save_path, period=1, num_frames=10, dataset='original', label=None, mask_path=None, padding=PADDING):
    ...
    tqdm.write(f'Processing video --- {org_path}')
    ...
    tqdm.write(f"N frame count --- {frame_count_org}")
...
if __name__=='__main__':
    ...
    if args.dataset=='Original':
        ...
        if 'v1' in SAVE_DIR:
            dataset_path='{}/Celeb-DF/'.format(ROOT)
        else:
            dataset_path='{}/Celeb-DF-v2/'.format(ROOT)
    ...
    print("{} : videos are exist in {} {} {}".format(len(movies_path_list), args.dataset, movies_path, file_list))
